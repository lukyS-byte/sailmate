...
function AddExpenseModal({ voyage, onClose, expense, rates }) {
  ...
  const czk = form.amount && rates && form.currency !== 'CZK'
    ? convert(parseFloat(form.amount), form.currency, 'CZK', rates)
    : null
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <div className="flex gap-1.5">
            <input className="input flex-1 min-w-0" type="number" step="1" min="0" placeholder="0" value={form.amount} onChange={f('amount')} required autoFocus />
          ...
          {czk !== null && (
            <p className="text-xs text-slate-400 mt-1">≈ {formatCurrency(czk, 'CZK')}</p>
          )}
        </div>
        <div>
